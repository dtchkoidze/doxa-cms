...
class Registration
{
    ...
    protected function getSuccessAuthUrl_OLD()
    {
        Clog::write(self::LOG, 'getSuccessAuthUrl()', Clog::DEBUG);
        ...
        if(method_exists($_profile, 'getSuccessAuthUrl')) {
            Clog::write(self::LOG, 'UserProfile exists in project', Clog::DEBUG);
            $success_auth_url = mr('user_profile')->getSuccessAuthUrl($this->user);
            Clog::write(self::LOG, '$this->success_auth_url from project UserProfile::getSuccessAuthUrl(): '.$success_auth_url, Clog::DEBUG);
        }

        if(!$success_auth_url){
            $success_auth_url = $this->getSuccessAuthUrlCookie();
            Clog::write(self::LOG, 'UserProfile NOT exists in project OR returned $this->success_auth_url = false, try to find in cookie: '.$success_auth_url, Clog::DEBUG);
            
            if($success_auth_url) {
                //$this->success_auth_url = $success_auth_url;
                Clog::write(self::LOG, 'success_auth_url exists in cookie: '.$success_auth_url, Clog::DEBUG);
            } else {
                $success_auth_url = '/';
                Clog::write(self::LOG, 'success_auth_url NOT exists in cookie, set to /', Clog::DEBUG);
            }
        }
